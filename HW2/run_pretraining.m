...
function main()
	...
	config.model = 'denoising';			% rbm, autoencoder, denoising
	...
	if config.resume
		...
		if config.pretrain
			if strcmp(config.model, 'rbm')
				load('rbm_trained_data.mat', 'rbm_weight');
				fc_weight.W{1} = rbm_weight.W;
			elseif strcmp(config.model, 'autoencoder')
				load('autoencoder_trained_data.mat', 'aenn_weight');
				fc_weight.W{1} = aenn_weight.W;
			elseif strcmp(config.model, 'denoising')
				load('denoising_trained_data.mat', 'aenn_weight');	
				fc_weight.W{1} = aenn_weight.W;
			else
				assert(false, 'model %s is not supported for pre-training.', config.model);
			end
			fprintf('initializing the parameters from file %s\n', config.model);
		end
		...
		if config.check_grad
			...
			for layer_index = 1:num_layer
				...
				for check_index = 1:config.num_check           % check each weight individually
					...
					if error_tmp > epsilon
						fprintf('gradient error for weights!!! %.5f', error_tmp);
						keyboard;
					end
				end
			end
			...
			for layer_index = 1:num_layer
				...
				for check_index = 1:config.num_check           % check each weight individually
					...
					if error_tmp > epsilon
						fprintf('gradient error for bias!!!');
						keyboard;
					end
				end
			end
		...
		for j = 1:config.num_epoch
			diary off;
			fc_weight = train_fc(fc_weight, train_image_data, train_labels_matrix, config.train, config.debug_mode);

			[train_accuracy(j), train_loss_avg(j)] = eval_classification_fc(fc_weight, train_image_data, train_labels_matrix, config, config.debug_mode);
			[valid_accuracy(j), valid_loss_avg(j)] = eval_classification_fc(fc_weight, valid_image_data, valid_labels_matrix, config, config.debug_mode);
	...
	fprintf('\n\n############################################### testing ###############################################\n\n');
	[test_accuracy, test_loss_avg] = eval_classification_fc(fc_weight, test_image_data, test_labels_matrix, config, config.debug_mode);
