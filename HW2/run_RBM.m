...
function main()
	...
	if config.resume
		...
		% else						
		config.rbm = struct();
		config.rbm.num_hidden = config.num_units;
		config.rbm.num_visible = config.im_width * config.im_height;

		% num_layer = length(config.net) - 1;
		rbm_weight = weights_initialization_rbm(config.rbm, config.initialization_method, config.debug_mode);	
		fprintf('restricted Boltzmann machine is:\n'); disp(config.rbm);
		fprintf('model has %d hidden variables and %d visible variables\n', config.rbm.num_hidden, config.rbm.num_visible);
		...
		for j = 1:config.num_epoch
			...
			if config.vis
				vis_weight = rbm_weight.W;		% 100 x 784
			...
			rbm_weight = train_rbm(rbm_weight, train_image_data, config.train, config.debug_mode);

			[train_loss_avg(j), ~] = eval_reconstruction_error_rbm(rbm_weight, train_image_data, 1, config.debug_mode);
			[valid_loss_avg(j), ~] = eval_reconstruction_error_rbm(rbm_weight, valid_image_data, 1, config.debug_mode);
		...
		save(save_path, 'rbm_weight', 'config', 'train_loss_avg', 'valid_loss_avg');
	end

	fprintf('\n\n############################################### testing ###############################################\n\n');
	[test_loss_avg, ~] = eval_reconstruction_error_rbm(rbm_weight, test_image_data, 1, config.debug_mode);
	...
	[~, reconstruction_data] = eval_reconstruction_error_rbm(rbm_weight, random_image, config.test.sampling_step, config.debug_mode);
	if config.vis
